...
class MessageCodecSuite extends munit.FunSuite:
  ...
  test("round trips common resource record data") {
    val records = Vector(
      ...
      ResourceRecord(example, RecordClass.IN, 60, RecordData.MX(10, DomainName.unsafe("mail.example.com."))),
      ResourceRecord(example, RecordClass.IN, 60, RecordData.TXT(Vector("hello".getBytes.toVector))),
      ResourceRecord(example, RecordClass.IN, 60, RecordData.SRV(0, 5, 443, DomainName.unsafe("service.example.com.")))
    )
    val original = Message(7, Flags(response = true, authoritative = true), Vector(Question(example, RecordType.A)), records)
    assertEquals(MessageCodec.decode(MessageCodec.encode(original)), Right(original))
  }

  test("encoder compresses repeated owner names") {
    val message = Message(1, questions = Vector(Question(example, RecordType.A)), answers = Vector(
      ResourceRecord(example, RecordClass.IN, 30, RecordData.ipv4("192.0.2.1"))
    ))
  ...
  test("rejects pointer beyond the packet") {
    ...
    assertEquals(MessageCodec.decode(packet), Left(DecodeError.CompressionPointerOutOfBounds(255, 18)))
  }
  ...
  test("enforces configurable section count limits before looping") {
    ...
    assertEquals(MessageCodec.decode(packet, sectionLimit = 1), Left(DecodeError.CountLimit("question", 2, 1)))
  }

  private def hex(value: String): Array[Byte] = value.grouped(2).map(Integer.parseInt(_, 16).toByte).toArray
