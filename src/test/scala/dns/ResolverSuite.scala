...
class ResolverSuite extends munit.FunSuite:
  ...
  test("a cache hit avoids upstream I/O and adapts transaction metadata") {
    ...
    var calls = 0
    val upstream =
      new Exchange[[value] =>> Either[DnsClient.Error, value]]:
        def apply(request: Message): Either[DnsClient.Error, Message] =
          calls += 1
          Right(request)
    val result = new CachingResolver(cache, upstream)
      .resolve(Message(99, Flags(recursionDesired = false), Vector(question)))
  ...
  test("a miss delegates and stores the response") {
    ...
    assertEquals(
      new CachingResolver(cache, upstream).resolve(Message(7, questions = Vector(question))),
      Right(response)
    )
