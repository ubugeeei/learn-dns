...
/** Parser for the common RFC 1035 master-file subset used by authoritative zones.
  *
  * Supported directives are `$ORIGIN` and `$TTL`; supported records are A,
  * AAAA, NS, CNAME, PTR, MX, TXT, SOA, and SRV. The parser understands relative
  * names, `@`, omitted owners, comments, quoted escapes, and parenthesized SOA.
  * All diagnostics include the originating logical line and are accumulated.
  */
object ZoneFile:
  ...
  def parse(
    ...
    if input.getBytes(StandardCharsets.UTF_8).length > config.maxInputBytes then
      ...
      ZoneFileSyntax.statements(input, config.maxLogicalLineLength).flatMap { statements =>
        parseStatements(statements, initialOrigin, config)
      }
  ...
  private def parseStatements(
    ...
    val finalState = statements.foldLeft(State(initialOrigin, None, None, Vector.empty, Vector.empty)) {
      (state, statement) =>
        if state.records.size >= config.maxRecords then
          state.copy(diagnostics = state.diagnostics :+ Diagnostic(
            statement.line,
            s"record count exceeds ${config.maxRecords}"
          ))
  ...
  private def parseDirective(
      state: State,
      statement: ZoneFileSyntax.Statement
  ): State = statement.tokens.map(_.toUpperCase) match
    ...
    case Vector("$TTL", _) =>
      ttl(statement.tokens(1), statement.line) match
        ...
        case Left(error)  => state.copy(diagnostics = state.diagnostics :+ error)
    case _ => state.copy(diagnostics = state.diagnostics :+ Diagnostic(
        statement.line,
    ...
    ownerResult match
      ...
      case Right(owner) =>
        ...
        parseRecordTail(owner, remainder, state, statement.line) match
          case Right(record) => state.copy(previousOwner = Some(owner), records = state.records :+ record)
          case Left(error) => state.copy(previousOwner = Some(owner), diagnostics = state.diagnostics :+ error)
  ...
  private def parseData(
    ...
    def exact(count: Int): Either[Diagnostic, Unit] =
      Either.cond(values.size == count, (), Diagnostic(line, s"$kind expects $count values"))
    def nameAt(index: Int): Either[Diagnostic, DomainName] =
      values.lift(index).toRight(Diagnostic(line, s"$kind is missing a name"))
        .flatMap(absoluteName(_, origin, line))
    def uint(index: Int, maximum: Int): Either[Diagnostic, Int] =
      values.lift(index).flatMap(_.toIntOption).filter(value => value >= 0 && value <= maximum)
    ...
    kind match
      ...
      case "MX" => for _ <- exact(2); preference <- uint(0, 0xffff); exchange <- nameAt(1)
        yield RecordData.MX(preference, exchange)
      case "TXT" =>
        Either.cond(values.nonEmpty, RecordData.TXT(values.map(_.getBytes(StandardCharsets.UTF_8).toVector)),
          Diagnostic(line, "TXT requires at least one string"))
      case "SRV" =>
        for _ <- exact(4); priority <- uint(0, 0xffff); weight <- uint(1, 0xffff)
  ...
  private def parseSoa(
    ...
    def number(index: Int): Either[Diagnostic, Long] =
      values.lift(index).toRight(Diagnostic(line, "SOA is missing a timer"))
        .flatMap(value => ttl(value, line))
    for
  ...
  private def buildZone(
      origin: DomainName,
      records: Vector[ResourceRecord]
  ): Either[Vector[Diagnostic], Zone] = records.find(record => record.name == origin && record.recordType == RecordType.SOA) match
    case None => Left(Vector(Diagnostic(1, s"zone has no SOA at $origin")))
    case Some(soa) => Zone.create(origin, soa, records.filterNot(_ eq soa)).left.map(error =>
        Vector(Diagnostic(1, s"zone validation failed: $error"))
      )
  ...
  private def absoluteName(
    ...
    val expanded = if value == "@" then origin.toString else if value.endsWith(".") then value
    else if origin.isRoot then s"$value." else s"$value.${origin.toString}"
    DomainName.fromString(expanded).left.map(error => Diagnostic(line, s"invalid name '$value': $error"))
  ...
  private def ttl(value: String, line: Int): Either[Diagnostic, Long] =
    ...
    val total = matches.foldLeft(Option(0L)) { (sum, part) =>
      for current <- sum; amount <- part.group(1).toLongOption
          next <- scala.util.Try(Math.addExact(current, Math.multiplyExact(amount, factors(part.group(2).toLowerCase)))).toOption
      yield next
    }
    total.filter(_ <= 0xffffffffL).filter(_ => complete)
      .toRight(Diagnostic(line, s"invalid TTL: $value"))

  private def address4(value: String, line: Int): Either[Diagnostic, RecordData] =
    scala.util.Try(RecordData.ipv4(value)).toEither.left.map(_ => Diagnostic(line, s"invalid IPv4 address: $value"))

  private def address6(value: String, line: Int): Either[Diagnostic, RecordData] =
    scala.util.Try(RecordData.ipv6(value)).toEither.left.map(_ => Diagnostic(line, s"invalid IPv6 address: $value"))
