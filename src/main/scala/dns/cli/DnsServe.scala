...
object DnsServe:
  ...
  def main(args: Array[String]): Unit =
    parse(args.toVector).flatMap(load) match
      ...
      case Right((config, zone)) =>
        ...
        println(s"serving ${config.origin} on ${server.address.getHostString}:${server.address.getPort}")
        stopped.await()

  private[cli] def parse(args: Vector[String]): Either[String, Config] = args match
    case file +: originText +: options =>
      DomainName.fromString(originText).left.map(error => s"invalid origin: $error").flatMap { origin =>
        parseOptions(
          options,
  ...
  private[cli] def load(config: Config): Either[String, (Config, Zone)] =
    scala.util.Try(Files.readString(config.zoneFile, StandardCharsets.UTF_8)).toEither
      .left.map(error => s"cannot read ${config.zoneFile}: ${error.getMessage}").flatMap { input =>
        ZoneFile.parse(input, config.origin).left.map { diagnostics =>
          diagnostics.map(value => s"${config.zoneFile}:${value.line}: ${value.message}").mkString("\n")
  ...
  private def parseOptions(tokens: Vector[String], config: Config): Either[String, Config] =
    tokens match
      ...
      case "--bind" +: value +: tail =>
        scala.util.Try(InetAddress.getByName(value)).toEither.left.map(_ => s"invalid bind address: $value")
  ...
  private def port(value: String): Either[String, Int] =
    value.toIntOption.filter(number => number >= 0 && number <= 65535)
      .toRight("port must be between 0 and 65535")

  val usage: String =
    "Usage: dns.cli.DnsServe <zone-file> <origin> [--bind address] " +
      "[--port n] [--udp-size n]"
