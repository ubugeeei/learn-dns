...
object DnsQuery:
  ...
  def main(args: Array[String]): Unit =
    parse(args.toVector) match
      case Left(error) =>
        ...
        sys.exit(2)
      case Right(config) =>
        val client = new DnsClient(
          config.server,
          config.timeout,
          config.udpPayloadSize,
          config.enableEdns
        )
  ...
  private def parseOptions(tokens: Vector[String], config: Config): Either[String, Config] =
    tokens match
      ...
      case "--timeout-ms" +: value +: tail =>
        positiveInt(value, "timeout").flatMap(milliseconds =>
          parseOptions(tail, config.copy(timeout = milliseconds.millis))
        )
      case "--udp-size" +: value +: tail =>
        positiveInt(value, "UDP size").flatMap { size =>
          Either.cond(
            size >= 512 && size <= 65535,
            size,
            "UDP size must be between 512 and 65535"
          )
  ...
  private def recordType(value: String): Either[String, RecordType] = value.toUpperCase match
  ...
  private def socket(value: String): Either[String, InetSocketAddress] =
    ...
    portText.map(_.toIntOption).getOrElse(Some(53))
      .filter(number => number > 0 && number <= 65535)
      .map(number => new InetSocketAddress(host, number))
      .toRight(s"invalid server address: $value")

  private def positiveInt(value: String, name: String): Either[String, Int] =
    value.toIntOption.filter(_ > 0).toRight(s"$name must be a positive integer")
