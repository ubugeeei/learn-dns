...
/** Validated bootstrap addresses extracted from an RFC-style root hints file.
  *
  * Only address records whose owners occur in the root NS RRset are accepted;
  * unrelated records cannot become bootstrap servers.
  */
...
object RootHints:
  ...
  def parse(input: String): Either[Error, RootHints] =
    ZoneFile.parseRecords(input, DomainName.Root).left.map(Error.Syntax.apply).flatMap { records =>
      val nameServers = records.collect {
